(function () {
        ...
        function update() {
          ...
          for (const plat of platforms) {
            const playerNextX = { ...player, x: nextX };
            const playerNextY = { ...player, y: nextY };
          ...
          if (player.x + player.width > canvas.width)
            (player.x = canvas.width - player.width), (player.dx = 0);
          ...
          for (let i = enemies.length - 1; i >= 0; i--) {
            ...
            if (!enemy.isStunned) {
              ...
              if (horizontalDist > 0) enemy.direction = 'right';
              else enemy.direction = 'left';
              if (
                Math.abs(horizontalDist) > 5 &&
                Math.abs(horizontalDist) < ATTACK_RANGE
              )
                enemy.dx = enemy.speed * Math.sign(horizontalDist);
            ...
            enemy.onWall = false;
            for (const plat of platforms) {
              const enemyNextXBox = { ...enemy, x: enemyNextX };
              const enemyNextYBox = { ...enemy, y: enemyNextY };
              if (checkCollision(enemyNextXBox, plat)) {
                if (enemy.dx > 0)
                  (enemyNextX = plat.x - enemy.width), (enemy.onWall = 'right');
                else if (enemy.dx < 0)
                  (enemyNextX = plat.x + plat.width), (enemy.onWall = 'left');
                enemy.dx = 0;
              }
              if (checkCollision(enemyNextYBox, plat)) {
                if (enemy.dy > 0)
                  ...
                  (enemyNextY = plat.y + plat.height), (enemy.dy = 0);
              }
            }
            enemy.x = enemyNextX;
            enemy.y = enemyNextY;
            ...
            if (enemy.x + enemy.width > canvas.width)
              (enemy.x = canvas.width - enemy.width), (enemy.dx = 0);
          ...
          for (let i = pellets.length - 1; i >= 0; i--) {
            ...
            const pelletBox = { x: p.x - 2, y: p.y - 2, width: 4, height: 4 };
        ...
        function draw() {
          ...
          for (const enemy of enemies) {
            ...
            ctx.fillRect(barX, barY, barWidth * healthPercentage, barHeight);
            if (enemy.isStunned) ctx.fillStyle = '#800080';
            else if (enemy.onWall) ctx.fillStyle = '#ff9999';
